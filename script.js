    ...
    new Chart(canvas, {
        ...
        data: {
            ...
            datasets: [{
                ...
                borderColor: Array.isArray(color) ? null : color,
                borderWidth: 1
            }]
        },
        options: {
...
function initSettingsModal() {
    ...
    if(openBtn) openBtn.addEventListener('click', () => { renderSettingsList(); modal.style.display = 'flex'; });
    if(closeBtn) closeBtn.addEventListener('click', () => { modal.style.display = 'none'; });
    if(saveBtn) saveBtn.addEventListener('click', saveSettings);
    if(addBtn) addBtn.addEventListener('click', addSettingItem);
}
        ...
        div.innerHTML = `
            ...
            <div style="flex:1;">
                ...
                <div style="display:flex; align-items:center; gap:5px; font-size:11px; color:#888;">
                    <span>Key:</span><input type="text" class="setting-key" value="${item.key}" data-index="${index}" placeholder="列名" style="border:none; background:#f9f9f9; padding:2px 4px; border-radius:3px; color:#666; width:100px;">
                    <span>Icon:</span><input type="text" class="setting-icon" value="${item.icon}" data-index="${index}" placeholder="fa-xx" style="border:none; background:#f9f9f9; padding:2px 4px; border-radius:3px; color:#666; width:80px;">
                </div>
            </div>
            ...
            <button onclick="deleteSettingItem(${index})" style="border:none; background:none; cursor:pointer; color:#e02424; margin-left:5px; font-size:14px;"><i class="fa-solid fa-trash-can"></i></button>
        `;
...
function addSettingItem() {
    currentConfig.push({ key: 'new_column', label: '新しい項目', icon: 'fa-circle', color: 'blue', format: 'number', position: 'sub' });
    renderSettingsList();
    const list = document.getElementById('settings-list');
...
window.deleteSettingItem = function(index) {
    if(confirm('削除しますか？')) { currentConfig.splice(index, 1); renderSettingsList(); }
};
...
async function saveSettings() {
    ...
    for(let i=0; i<rows.length; i++) {
        ...
        newConfig.push({ key: keyInput.value.trim(), label: labelInput.value.trim(), icon: iconInput.value.trim(), color: base.color || 'blue', format: base.format || 'number', position: posSelect.value });
    }
    const saveBtn = document.getElementById('save-settings-btn');
    saveBtn.textContent = "保存中...";
    saveBtn.disabled = true;
    try {
        const response = await fetch(GAS_API_URL, { method: "POST", mode: "cors", redirect: "follow", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify({ token: currentToken, action: 'updateSettings', settings: newConfig }) });
        const jsonData = await response.json();
        if(jsonData.success) { alert('設定を保存しました'); document.getElementById('settings-modal').style.display = 'none'; fetchData('getSales'); }
        else { alert('保存エラー: ' + jsonData.message); }
    } catch(e) { console.error(e); alert('通信エラー'); } finally { saveBtn.textContent = "保存して反映"; saveBtn.disabled = false; }
}
...
function initUploadModal() {
    ...
    dropArea.addEventListener('drop', (e) => { e.preventDefault(); dropArea.style.borderColor = '#ccc'; dropArea.style.background = '#fafafa'; if (e.dataTransfer.files.length > 0) handleFile(e.dataTransfer.files[0]); });
    fileInput.addEventListener('change', (e) => { if (e.target.files.length > 0) handleFile(e.target.files[0]); });
    function handleFile(file) { selectedFile = file; dropArea.style.display = 'none'; previewArea.style.display = 'block'; fileName.textContent = `${file.name} (${(file.size/1024).toFixed(1)} KB)`; executeBtn.disabled = false; executeBtn.style.background = 'var(--accent-blue)'; executeBtn.style.cursor = 'pointer'; }
    function resetUploadForm() { selectedFile = null; fileInput.value = ''; dropArea.style.display = 'block'; previewArea.style.display = 'none'; executeBtn.disabled = true; executeBtn.style.background = '#ccc'; executeBtn.textContent = "AI解析スタート"; }
    const compressImage = (file) => { return new Promise((resolve, reject) => { const MAX_WIDTH = 1024; const MAX_HEIGHT = 1024; const reader = new FileReader(); reader.readAsDataURL(file); reader.onload = (event) => { const img = new Image(); img.src = event.target.result; img.onload = () => { let width = img.width; let height = img.height; if (width > height) { if (width > MAX_WIDTH) { height *= MAX_WIDTH / width; width = MAX_WIDTH; } } else { if (height > MAX_HEIGHT) { width *= MAX_HEIGHT / height; height = MAX_HEIGHT; } } const canvas = document.createElement('canvas'); canvas.width = width; canvas.height = height; const ctx = canvas.getContext('2d'); ctx.drawImage(img, 0, 0, width, height); const dataUrl = canvas.toDataURL('image/jpeg', 0.8); resolve({ base64: dataUrl.split(',')[1], mimeType: 'image/jpeg' }); }; img.onerror = reject; }; reader.onerror = reject; }); };
    const readPdf = (file) => { return new Promise((resolve, reject) => { const reader = new FileReader(); reader.readAsDataURL(file); reader.onload = () => resolve({ base64: reader.result.split(',')[1], mimeType: file.type }); reader.onerror = reject; }); };
...
function initInputModal() {
    ...
    if(openBtn) { openBtn.addEventListener('click', () => { const today = new Date(); let defaultDate = new Date(selectedDate); if (today.getMonth() === selectedDate.getMonth() && today.getFullYear() === selectedDate.getFullYear()) { defaultDate = today; } const y = defaultDate.getFullYear(); const m = ('0' + (defaultDate.getMonth() + 1)).slice(-2); const d = ('0' + defaultDate.getDate()).slice(-2); dateInput.value = `${y}-${m}-${d}`; renderInputForm(); modal.style.display = 'flex'; }); }
    if(closeBtn) { closeBtn.addEventListener('click', () => { modal.style.display = 'none'; }); }
    ...
    if(saveBtn) { saveBtn.addEventListener('click', async () => { const targetDate = dateInput.value; if(!targetDate) { alert('日付を選択してください'); return; } const inputData = { date: targetDate }; const inputs = formContainer.querySelectorAll('input.input-field-dynamic'); inputs.forEach(input => { const key = input.dataset.key; const val = input.value; if (val !== '') { inputData[key] = Number(val); } }); saveBtn.textContent = '保存中...'; saveBtn.disabled = true; try { const response = await fetch(GAS_API_URL, { method: "POST", mode: "cors", redirect: "follow", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify({ token: currentToken, action: 'manualInput', data: inputData }) }); const jsonData = await response.json(); if(jsonData.success) { alert('データを保存しました'); modal.style.display = 'none'; fetchData('getSales'); } else { alert('エラー: ' + jsonData.message); } } catch(e) { console.error(e); alert('通信エラー'); } finally { saveBtn.innerHTML = '<i class="fa-solid fa-save"></i> 保存する'; saveBtn.disabled = false; } }); }
}
...
async function fetchData(actionType) {
    ...
    try {
        const response = await fetch(GAS_API_URL, { method: "POST", mode: "cors", redirect: "follow", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify({ token: currentToken, action: actionType, targetMonth: getSelectedMonthString() }) });
        if (!response.ok) throw new Error('Network error');
        const jsonData = await response.json();
        if (jsonData.error) { console.error("API Error:", jsonData.message); alert(jsonData.message); return; }
...
function renderSalesDashboard(data) {
    ...
    config.forEach(item => {
        ...
        let unit = '', displayVal = '';
        if (item.format === 'currency') { displayVal = Number(val).toLocaleString(); unit = '¥'; } else if (item.format === 'percent') { displayVal = Number(val).toFixed(1); unit = '%'; } else { displayVal = Number(val).toLocaleString(); }
        let trendHtml = '';
        let diff = val - prevVal;
        let diffPercent = 0;
        if (prevVal !== 0) { diffPercent = ((val - prevVal) / prevVal) * 100; }
        const isCancelMetric = item.key.includes('cancel');
        const isUp = diff >= 0;
        let trendClass = 'flat'; let trendIcon = 'fa-minus'; let trendText = '前月比 ±0';
        if (diff !== 0) { const isGood = isCancelMetric ? !isUp : isUp; trendClass = isGood ? 'up' : 'down'; trendIcon = isUp ? 'fa-arrow-trend-up' : 'fa-arrow-trend-down'; const sign = isUp ? '+' : ''; if (item.format === 'percent') { trendText = `前月比 ${sign}${diff.toFixed(1)}pt`; } else { trendText = `前月比 ${sign}${diffPercent.toFixed(1)}%`; } }
        trendHtml = `<span class="trend ${trendClass}"><i class="fa-solid ${trendIcon}"></i> ${trendText}</span>`;
        const cardHtml = `<div class="card kpi-card"><div class="card-icon ${item.color}"><i class="fa-solid ${item.icon}"></i></div><div class="card-info"><h3>${item.label}</h3><p class="value">${unit === '¥' ? '¥' : ''}${displayVal}${unit === '%' ? '%' : ''}</p>${trendHtml}</div></div>`;
        if (item.position === 'top') { if(topContainer) topContainer.insertAdjacentHTML('beforeend', cardHtml); } else { if(subContainer) subContainer.insertAdjacentHTML('beforeend', cardHtml); }
    });
    ...
    if (chartStatus) chartStatus.destroy();
    if (!history) return;
    const labels = history.map(item => { const d = new Date(item.date); return d.getDate() + '日'; });
...
function updateAiAdvisor(summary, prevSummary) {
    ...
    if (!summary || !prevSummary) { aiTextElem.textContent = "データが不足しているため分析できません。"; return; }
    ...
    let messages = [];
    if (sales > prevSales * 1.05) { messages.push(`素晴らしいです！売上が前月比で増加傾向にあります（+${Math.round((sales - prevSales)/10000)}万円）。この調子で自費診療のアポイントも確保していきましょう。`); } else if (sales < prevSales * 0.95) { messages.push(`売上が前月より少し落ち込んでいます。空き枠の確認と、リコール患者様へのアプローチを検討してみましょう。`); } else { messages.push(`売上は前月と同水準で安定しています。`); }
    if (cancelRate > 15) { messages.push(`⚠️ キャンセル率が ${cancelRate.toFixed(1)}% と高まっています。前日の確認連絡を強化するか、無断キャンセルの多い患者様の予約管理を見直す必要があります。`); } else if (cancelRate < 5 && visitors > 20) { messages.push(`キャンセル率が低く抑えられています（${cancelRate.toFixed(1)}%）。患者様との信頼関係が構築できている証拠です。スタッフを労いましょう！`); }
    if (selfPayRate < 20) { messages.push(`自費率が ${selfPayRate.toFixed(1)}% です。カウンセリングの機会を増やし、補綴物の選択肢を提示する回数を増やしてみても良いかもしれません。`); }
    let finalMessage = messages.join('<br><br>');
    if (sales === 0 && visitors === 0) { finalMessage = "まだ今月のデータが入力されていないようです。日計表をアップロードするか、スプレッドシートに入力してください。"; }
...
function renderPhrDashboard(data) {
    ...
    new Chart(canvas, { type: 'line', data: { labels: labels, datasets: [ { label: 'PHR記録アクション数', data: counts, borderColor: '#10b981', backgroundColor: 'rgba(16, 185, 129, 0.1)', borderWidth: 3, fill: true, tension: 0.4 } ] }, options: { responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: '日別 PHR利用アクティビティ' } }, scales: { y: { beginAtZero: true } } } });
}
